...
from src.agents.llm.llm_adapter import LLMAdapter
from src.agents.enum.agent_state import AgentState  # 这是“认知态”

from src.agents.core.base_message import Message
from src.agents.core.prompt_agent import PromptAgent
from src.shared.exceptions.agent_errors import AgentExecutionError
...
class SimpleAgent(PromptAgent):
    """
    SimpleAgent = 最基础的执行型 Agent

    职责：
    - 调用 PromptAgent.build_messages()
    - 选择 LLM Adapter 执行
    - 支持同步 / 流式输出
    ...
    def __init__(
        ...
        # 认知态（不影响 BaseAgent 的 runtime_state）
        self.cognitive_state: Optional[AgentState] = None
    ...
    async def _process(
        ...
        """
        BaseAgent 唯一允许子类实现的方法
        """

        try:
            # ---- 1. 进入认知态（可选，但推荐） ----
            self.cognitive_state = AgentState.PROCESSING

            # ---- 2. 构建 Messages（来自 PromptAgent） ----
            messages = self.build_messages(input_data, **kwargs)

            # ---- 3. 执行 LLM ----
            if stream:
                return self._stream_execute(messages, **kwargs)
            else:
                return await self._execute(messages, **kwargs)

        except Exception as e:
            self.cognitive_state = AgentState.ERROR
            logger.exception("SimpleAgent execution failed")
    ...
    async def _execute(
        ...
        非流式执行
        """
        self.cognitive_state = AgentState.THINKING
    ...
    async def _stream_execute(
        ...
        async for chunk in self.llm.stream(messages, **kwargs):
            yield chunk
